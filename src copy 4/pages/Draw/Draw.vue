  ...
  <div class="draw">
        ...
        <div class="right">
          <div v-for="(item,index) in drawRounds" :key="item.round">
            ...
            <div class="persons">
              ...
              <div v-for="(item,index) in showAllWinUser[item.round]" :key="index">
                <img :src="item.avatar" alt="">
              </div>
              <!-- <div v-for="(item,index) in showAllWinUser[Object.keys(winUser)[index]-1]" :key="index">
                <img :src="item.avatar" alt="">
              </div> -->
            </div>
          </div>
    ...
    <div class="drawFooter">
      <div class="btns">
        ...
        <button class="deleteRound" @click="deleteRound" >{{deleteRoundText}}</button>
      ...
      <div class="roundBox" v-show="showRoundBox">
        ...
        <div class="selfRound">
          <div>
            <span>第</span>
            <input type="number" name="" v-model="round" min="1">
            <span>轮</span>
            <input type="number" name="" v-model="persons" min="1">
            <span>人</span>
          </div>
      ...
      <!-- 删除轮次 -->
      <div class="deleteRoundBox" v-show="showIsDeleteRound">
        <div class="deleteAll">
          <span>全部删除</span>
          <input type="checkbox" name="checkbox" class="checkbox" v-model="deleteAllRound">
        </div>
        <div v-for="(item,index) in drawRounds" :key="item.round">
          <span>删除第{{item.round}}轮</span>
          <input type="checkbox" name="checkbox" class="checkbox" v-model="item.isClear">
        </div>
      </div>
    </div>
  ...
  export default {
    ...
    data() {
      return {
        ...
        clearRoundText:'清空轮次',
        showIsDeleteRound:false,
        deleteRoundText:'删除轮次',
        //自定义的轮次和人数 v-model
        round:'',
        persons:'',
        selfId:'',
    ...
    methods: {
      ...
      async deployRound(){
        ...
        this.showIsDeleteRound = false
        this.clearRoundText = '清空轮次'
        // this.drawRounds.length>0 && (this.roundArr = this.drawRounds)
        this.roundArr = JSON.parse(JSON.stringify(this.drawRounds))
      ...
      async reqSetRound(){
        ...
        localStorage.setItem('allWinNum',this.allWinNum)
        //配置请求后台需要的参数
      ...
      async start(){
        ...
        if (this.satrtAndOver === '开始') {
          // 抽奖是否已完成
          if(this.allWinNum == this.showAllWinUserNum){
            this.drawUrl = ''
            return
          }
          this.satrtAndOver = '停'
          ...
          if (reqRoundId) {
            ...
            if (code == 1) {
              ...
              if (this.drawRounds[this.roundIndex-2]) {
                let roundId = this.drawRounds[this.roundIndex-2].round
                this.winUser[roundId] = data
          ...
          //取出每轮中奖用户列表
          if (this.drawRounds[this.winUserIndex]) {
            let roundId = this.drawRounds[this.winUserIndex].round
            this.roundData = this.winUser[roundId]
            ...
            if (this.roundData) {
              ...
              if (key) {
                ...
                this.showAllWinUser[roundId] = this.showPerWinUser
              ...
              if (code == 1) {
                ...
                clearRoundInfo.forEach((item)=>{
                  if (this.roundNameArr) {
                    ...
                    // this.showAllWinUser[a] = []
                    this.showAllWinUser[item.round] = []
                    delete this.winUser[item.round]
                  }
                })
                localStorage.setItem('winUserDatas', JSON.stringify(this.winUser))
                localStorage.setItem('showAllWinUserDatas', JSON.stringify(this.showAllWinUser))
      ...
      async deleteRound() {
        // console.log('删除轮次')
        !!!this.showIsDeleteRound ? (this.showIsDeleteRound = true) : (this.showIsDeleteRound = false)
        this.showIsResetDraw = false
        this.showRoundBox = false
        this.showIsClearRound = false
        if (this.deleteRoundText === '删除轮次' ) {
          this.deleteRoundText = '确定'
          this.deleteAllRound = false
          this.drawRounds = this.drawRounds.map((item)=>{
            item.isClear = false
            return item
          })
          ...
          this.deleteRoundText = '删除轮次'
          if (this.deleteAllRound === true) {
            this.showAllWinUser = {}
            this.showPerWinUser = []
            this.winUser = {}
            this.winUserIndex = 0
            ...
            localStorage.removeItem('allWinNum')
              ...
              if (code == 1) {
                ...
                clearRoundInfo.forEach((item)=>{
                  if (this.roundNameArr) {
                    ...
                    //清除右侧显示的中奖信息
                    // delete this.showAllWinUser[a]
                    delete this.showAllWinUser[item.round]
                    delete this.winUser[item.round]

                    _.remove(this.drawRounds,(n)=>{
                      return n.round == item.round
                    })
                    // this.drawRounds.splice(a,1)
                    // _.remove(this.roundIdArr)
                    // this.roundIdArr.splice(a,1)
                  }
                })
                localStorage.setItem('winUserDatas', JSON.stringify(this.winUser))
                localStorage.setItem('showAllWinUserDatas', JSON.stringify(this.showAllWinUser))
                ...
                Object.keys(this.showAllWinUser).forEach((item)=>{
                  this.showAllWinUserNum = this.showAllWinUserNum + this.showAllWinUser[item].length
                })
                localStorage.setItem('showAllWinUserNum',this.showAllWinUserNum)
                ...
                this.allWinNum = this.drawRounds.reduce((pre,cur)=>{
                  return pre + cur.persons*1
                },0)
                localStorage.setItem('allWinNum',this.allWinNum)


                // if (Object.keys(this.showAllWinUser).length>0) {
                if (this.showAllWinUserNum == this.winUserNum) {
                  let b = this.drawRounds.length
                  this.roundIndex = b+1
                  localStorage.setItem('roundIndex', this.roundIndex)
                  this.winUserIndex = b
                  localStorage.setItem('winUserIndex', this.winUserIndex)
    ...
    watch:{
      showPerWinUser(){
        ...
        if (this.showAllWinUserNum == this.allWinNum) {
          let b = this.drawRounds.length
          this.roundIndex = b+1
          localStorage.setItem('roundIndex', this.roundIndex)
          this.winUserIndex = b
    ...
    activated(){
      //签到用户列表
      !!this.getuserListTimeId && clearInterval(this.getuserListTimeId)
      this.getuserListTimeId = setInterval(() => {
      ...
      //中奖用户人数
      let allWinNum = localStorage.getItem('allWinNum')
      allWinNum && (this.allWinNum = allWinNum)
